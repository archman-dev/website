...
- Unit tests should verify object invariants and behavior through public methods. Avoid testing private state directly; prefer constructing valid objects and asserting outcomes.
- Use dependency injection to substitute collaborators with fakes/mocks. Verify interactions at interface boundaries rather than concrete types.
- Write contract tests for adapters that implement shared interfaces to ensure consistent behavior across implementations (e.g., different payment gateways).
- Prefer property-based tests for value objects (equality, ordering, serialization) and boundary conditions (min/max, empty/invalid).
- For concurrency, add stress/race tests and guard against shared mutable state. Inject clocks/ID generators to keep tests deterministic.
